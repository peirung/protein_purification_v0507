...
class Protocol
  ...
  def main
    ...
    operations.running.each do |op|
        ...
        op_wash_amount << op.output("Protein").sample.properties["Wash_Buffer"]
    end
  ...
  def buffer_preparation(op_count,op_in_cell_lysate,op_wash_amount, op_out_protein)
    ...
    for i in 0..(op_count-1)
        ...
        row << {content: "#{op_wash_amount[i]*15*1000/5000} µl", check: true}
        op_table << row
    end
  ...
  def add_elution_buff(op_count,op_in_resin,op_out_protein)
    ...
    show do
       ...
       table op_table_1
       check "Fully open the column oulet and make sample slowly flow out of the column (as show in the picture)."
